  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    collector.on('collect', async button => {
      if (button.user.id === cmduser.id) {
          ...
          await tempmsg.channel.awaitMessages({
            ...
            .then(collected => {
              ...
              try {
                ...
                return message.reply({
                  embeds: [new Discord.MessageEmbed()
                    .setTitle(`<:21:1070256823174713364> The new Embed Footer Text is:`.substr(0, 256))
                    .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
        ...
        content: `<:20:1070256826903445524> You are not allowed to do that! Only: <@${cmduser.id}>`,
        ephemeral: true
      });
    });
    ...
    collector.on('end', collected => {
      tempmsg.edit({
        ...
        components: [],
        content: `${collected && collected.first() && collected.first().customId ? `<:21:1070256823174713364> **Selected the \`${collected.first().customId}\`. Button**` : "❌ **NOTHING SELECTED - CANCELLED**" }`
      })
    });
  },
};
