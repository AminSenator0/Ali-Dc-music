...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    if (args[0]) {
      ...
      let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("1070256823174713364").setLabel(handlemsg(client.la[ls].cmds.info.help.buttons.back))
      ...
      let button_forward = new MessageButton().setStyle('SUCCESS').setCustomId('3').setEmoji('936940889950027786').setLabel(handlemsg(client.la[ls].cmds.info.help.buttons.forward))
      let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("🤖").setLabel("Sup").setURL("https://discord.gg/bht")
      let menuOptions = [{
          label: "Overview",
          value: "Overview",
      //emoji: "",
...
Made by [**black holes Development**](https://discord.gg/bht)`)
    ...
    function allotherembeds_eachcategory(filterdisabled = false) {
      ...
      var embed9 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<:dis:1182612226012618752> **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("info")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
